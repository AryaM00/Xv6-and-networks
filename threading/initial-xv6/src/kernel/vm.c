...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
    ...
    int status = cow_handler(pagetable, va0);
...
int cow_handler(pagetable_t pagetable,uint64 va){
  ...
  if(pte){
    ...
    if ((*pte) && PTE_COW) {
      ...
      // removing the cow flag and adding write flag
      flags |= PTE_W;
      flags &= ~PTE_COW;
    ...
    //   memset((void*)mem,0,sizeof(mem));
      ...
      memmove(mem,(char*)pa,PGSIZE); //copy the content of old page to new page

      uvmunmap(pagetable,PGROUNDUP(va),1,1); //unmap the old page
      mappages(pagetable,va,PGSIZE,(uint64)mem,flags); //map the new page with new flags
    }
    else return -1;
  }
    else return -1;
